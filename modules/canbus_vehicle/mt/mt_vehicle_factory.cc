...
#include "modules/canbus_vehicle/mt/mt_vehicle_factory.h"
...
bool MtVehicleFactory::Init(const CanbusConf *canbus_conf) {
  // Init can client
  ...
  chassis_detail_writer_ =
      node_->CreateWriter<::apollo::canbus::Mt>(FLAGS_chassis_detail_topic);

  return true;
}

bool MtVehicleFactory::Start() {
...
void MtVehicleFactory::UpdateCommand(
...
Chassis MtVehicleFactory::publish_chassis() {
  Chassis chassis = vehicle_controller_->chassis();
  ADEBUG << chassis.ShortDebugString();
  return chassis;
}

void MtVehicleFactory::PublishChassisDetail() {
  Mt chassis_detail;
...
void MtVehicleFactory::UpdateHeartbeat() {
    can_sender_.Update_Heartbeat();
}

std::unique_ptr<VehicleController<::apollo::canbus::Mt>>
MtVehicleFactory::CreateVehicleController() {
  return std::unique_ptr<VehicleController<::apollo::canbus::Mt>>(
      new mt::MtController());
}

std::unique_ptr<MessageManager<::apollo::canbus::Mt>>
MtVehicleFactory::CreateMessageManager() {
  return std::unique_ptr<MessageManager<::apollo::canbus::Mt>>(
      new mt::MtMessageManager());
}
